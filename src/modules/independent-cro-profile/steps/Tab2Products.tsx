...
import {
  ...
  EmptyState, DrawerFooter, TabFooter,
...
import { CRO_INDUSTRIES } from "../constants";
...
const MOQ_UNITS = ["g", "Kg", "MT", "L", "mL"] as const;
const QTY_UNITS = ["g", "Kg", "MT", "L"] as const;
...
function AddProductDrawer({ open, onClose, onSave }: {
  ...
  return (
    <DrawerBase open={open} onClose={onClose} title="Add Product" width={540}
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <FormField label={<>Product CAS Number <span className="text-red-500">*</span></>}>
            <input
              className={inputCls}
        ...
        <div className="flex flex-col gap-3 pt-1">
          <label className="flex items-start gap-3 cursor-pointer">
            <input
              type="checkbox"
              checked={form.crackedChemistry}
              onChange={(e) => set("crackedChemistry", e.target.checked)}
              className="w-4 h-4 rounded accent-[#1F6F54] mt-0.5"
            />
            <div>
              <p className="text-sm font-medium text-[#020202]">Cracked Chemistry</p>
              <p className="text-xs text-[#9ca3af]">We have successfully synthesized or reverse-engineered this compound.</p>
            </div>
          </label>
          <label className="flex items-start gap-3 cursor-pointer">
            <input
              type="checkbox"
              checked={form.workedOnProduct}
              onChange={(e) => set("workedOnProduct", e.target.checked)}
              className="w-4 h-4 rounded accent-[#1F6F54] mt-0.5"
            />
            <div>
              <p className="text-sm font-medium text-[#020202]">Worked on this Product</p>
              <p className="text-xs text-[#9ca3af]">Our team has hands-on experience working with this product in R&D or commercial production.</p>
            </div>
          </label>
        </div>
...
function ProductCard({ p, onDelete }: { p: CROProduct; onDelete: () => void }) {
...
export function Tab2Products({ onNext, onBack }: Props) {
  ...
  return (
    <>
      <AddProductDrawer open={open} onClose={() => setOpen(false)} onSave={addProduct} />
      <div className="flex flex-col">
        <div className="p-4 sm:p-6 pb-2">
          ...
          {products.length === 0
            ? <EmptyState icon={Package} title="No products added yet" subtitle="Click Add Product to list your chemicals and compounds." />
            : <CollapsibleCardGrid
                items={products}
